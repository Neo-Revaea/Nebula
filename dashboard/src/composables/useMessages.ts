      ...
      while (true) {
        try {
          if (
            disposed ||
            mySendSeq !== sendSeq ||
            sendSessionId !== currSessionId.value
          ) {
            try {
              await reader.cancel();
            } catch {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (
              !chunk_json ||
              typeof chunk_json !== 'object' ||
              !chunk_json.hasOwnProperty('type')
            ) {
            ...
            if (chunk_json.type === 'image') {
              ...
              if (chain_type === 'tool_call') {
                ...
                if (!in_streaming) {
                  ...
                  const lastPart =
                    message_obj!.message[message_obj!.message.length - 1];
                ...
                if (!in_streaming) {
                  ...
                  message_obj!.reasoning =
                    (message_obj!.reasoning || '') + chunk_json.data;
                }
              } else {
                // normal text
                if (!in_streaming) {
            ...
            if (
              (chunk_json.type === 'break' && chunk_json.streaming) ||
              !chunk_json.streaming
            ) {
              in_streaming = false;

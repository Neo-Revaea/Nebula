...
export function usePluginMarket({
  ...
  const toPinyinText = (s: unknown) =>
    pinyin(String(s ?? ''), { toneType: 'none' })
      .toLowerCase()
      .replace(/\s+/g, '');
  const toInitials = (s: unknown) =>
    pinyin(String(s ?? ''), { pattern: 'first', toneType: 'none' })
      .toLowerCase()
      .replace(/\s+/g, '');

  const marketCustomFilter = (
    value: unknown,
    query: string,
  ...
  const checkAlreadyInstalled = () => {
    ...
    const installedRepos = new Set(
      installed.map((ext) => ext.repo?.toLowerCase()),
    );
  ...
  const totalPages = computed(() =>
    Math.ceil(sortedPlugins.value.length / displayItemsPerPage),
  );

...
export function useInstalledPlugins({
  ...
  const filteredExtensions = computed(() => {
    ...
    const sorted = [...data].sort((a, b) => {
      const aKey = (
        a.display_name?.length ? a.display_name : a.name || ''
      ).toLowerCase();
      const bKey = (
        b.display_name?.length ? b.display_name : b.name || ''
      ).toLowerCase();
  ...
  const getExtensions = async () => {
    ...
    try {
      const res =
        await axios.get<ApiResponse<InstalledPlugin[]>>('/api/plugin/get');
  ...
  const pluginOn = async (extension: InstalledPlugin) => {
    try {
      const res = await axios.post<ApiResponse<unknown>>('/api/plugin/on', {
        name: extension.name,
      });
  ...
  const pluginOff = async (extension: InstalledPlugin) => {
    try {
      const res = await axios.post<ApiResponse<unknown>>('/api/plugin/off', {
        name: extension.name,
      });
      if (res.data.status === 'error') {
        toast(res.data.message, 'error');
  ...
  const reloadPlugin = async (plugin_name: string) => {
    try {
      const res = await axios.post<ApiResponse<unknown>>('/api/plugin/reload', {
        name: plugin_name,
      });
      if (res.data.status === 'error') {
        toast(res.data.message, 'error');
  ...
  const uninstallExtension = async (
    ...
    try {
      const res = await axios.post<ApiResponse<InstalledPlugin[]>>(
  ...
  const updateExtension = async (
    ...
    if (!forceUpdate) {
      const data = Array.isArray(extension_data?.data)
        ? extension_data.data
        : [];
  ...
  const updateAllExtensions = async () => {
    ...
    try {
      const res = await axios.post<
        ApiResponse<{ results?: UpdateAllResult[] }>

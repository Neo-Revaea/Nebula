<template>
  <div class="message-parts-renderer" :style="themeStyle">
    <template v-for="renderPart in getRenderParts(parts)" :key="renderPart.key">
      ...
      >
        <template #label="{ expanded }">
          ...
          <span style="opacity: 0.6">{{
            renderPart.toolCall.finished_ts
              ? formatDuration(
                  renderPart.toolCall.finished_ts - renderPart.toolCall.ts,
                )
              : getElapsedTime(renderPart.toolCall.ts)
          }}</span>
          <v-icon
            size="small"
            class="ipython-icon"
      ...
      <div
        v-else-if="
          renderPart.part.type === 'reply' && renderPart.part.message_id != null
        "
        class="reply-quote"
      ...
      >
        ...
        <span class="reply-quote-text">{{
          getReplyContentText(renderPart.part.message_id)
        }}</span>
      </div>
      ...
      <MarkdownRender
        ...
        :monaco-options="{
          theme: isDark || isMarkdownDark ? 'vs-dark' : 'vs-light',
        }"
      />
      ...
      >
        <div class="embedded-file">
          ...
          >
            ...
            <v-icon
              v-if="
                renderPart.part.embedded_file.attachment_id != null &&

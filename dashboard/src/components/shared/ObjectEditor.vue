...
function initializeLocalKeyValuePairs() {
  ...
  for (const [key, value] of Object.entries(modelValue)) {
    const inferredType: ValueType =
      typeof value === 'object' ? 'json' : (typeof value as ValueType);
    ...
    if (template) {
      ...
      if (pairValue === undefined || pairValue === null) {
        pairValue =
          (template.default as string | number | boolean | null | undefined) ??
          pairValue;
      }
    }
...
function openDialog() {
  ...
  originalKeyValuePairs.value = JSON.parse(
    JSON.stringify(localKeyValuePairs.value),
  ); // Deep copy
  newKey.value = '';
...
function updateKey(index: number, newKey: string) {
  ...
  // 检查新键名是否已存在
  const isKeyExists = localKeyValuePairs.value.some(
    (pair, i) => i !== index && pair.key === newKey,
  );
  if (isKeyExists) {
    // 如果键名已存在，提示用户并恢复原值
    alert('键名已存在');
  ...
  if (template) {
    // 更新类型和默认值
    localKeyValuePairs.value[index].type =
      (template.type as ValueType | undefined) ??
      localKeyValuePairs.value[index].type;
    if (
      localKeyValuePairs.value[index].value === undefined ||
      localKeyValuePairs.value[index].value === null ||
    ...
    localKeyValuePairs.value[index].slider = template.slider as
      | SliderSpec
      | undefined;
...
function getTemplateValue(
  ...
  return (
    ...
    getDefaultValueForType(
      (template?.type as ValueType | undefined) ?? 'string',
    )
  );
}
...
function cancelDialog() {
  ...
  localKeyValuePairs.value = JSON.parse(
    JSON.stringify(originalKeyValuePairs.value),
  );
  dialog.value = false;
}
</script>

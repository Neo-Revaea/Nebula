<template>
  <div>
    <v-row class="mb-4">
      <v-col cols="12" class="d-flex align-center flex-wrap ga-2">
        ...
        <v-btn
          class="ml-2"
          variant="tonal"
          @click="emit('toggle-show-reserved')"
        >
        ...
        <v-col cols="12" sm="auto" class="ml-auto">
          <v-dialog v-if="extensionMessage" max-width="500px">
          ...
          >
            ...
            <template #item.name="{ item }">
              <div class="d-flex align-center py-2">
                ...
                <div>
                  ...
                  <div
                    v-if="hasExtensionDisplayName(item)"
                    class="text-caption text-medium-emphasis mt-1"
                  >
                    {{ resolveRow(item).name }}
                  </div>
                  <div
                    v-if="resolveRow(item).reserved"
            ...
            <template #item.version="{ item }">
              <div class="d-flex align-center">
                ...
                <v-icon
                  v-if="resolveRow(item).has_update"
                  color="warning"
                  size="small"
                  class="ml-1"
                >
                ...
                >
                  <span
                    ...
                    {{ resolveRow(item).online_version }}</span
                  >
                </v-tooltip>
              </div>
            </template>
            ...
            <template #item.author="{ item }">
              <div class="text-body-2">
                {{ resolveRow(item).author }}
              </div>
            </template>

            <template #item.activated="{ item }">
              <v-chip
                :color="resolveRow(item).activated ? 'success' : 'error'"
                size="small"
                class="font-weight-medium"
                :variant="resolveRow(item).activated ? 'flat' : 'outlined'"
              >
                {{
                  resolveRow(item).activated
                    ? tm('status.enabled')
                    : tm('status.disabled')
                }}
              </v-chip>
            ...
            <template #item.actions="{ item }">
              <div class="d-flex align-center">
                ...
                >
                  <v-btn
                    v-if="!resolveRow(item).activated"
                    icon
                    size="small"
                    color="success"
                    @click="emit('plugin-on', resolveRow(item))"
                  >
                  ...
                  <v-btn
                    ...
                    size="small"
                    color="error"
                    @click="emit('plugin-off', resolveRow(item))"
                  >
                  ...
                  <v-btn
                    ...
                    size="small"
                    color="info"
                    @click="emit('reload', resolveRow(item).name)"
                  >
                  ...
                  <v-btn
                    icon
                    size="small"
                    @click="emit('open-config', resolveRow(item).name)"
                  >
                  ...
                  <v-btn
                    icon
                    size="small"
                    @click="emit('show-info', resolveRow(item))"
                  >
                  ...
                  <v-btn
                    v-if="resolveRow(item).repo"
                    icon
                    size="small"
                    @click="emit('view-readme', resolveRow(item))"
                  >
                  ...
                  <v-btn
                    icon
                    size="small"
                    @click="emit('view-changelog', resolveRow(item))"
                  >
                  ...
                  <v-btn
                    ...
                    size="small"
                    color="warning"
                    @click="emit('update-extension', resolveRow(item).name)"
                  >
                    <v-icon>mdi-update</v-icon>
                    <v-tooltip activator="parent" location="top">
                      {{ tm('tooltips.update') }}
                  ...
                  <v-btn
                    ...
                    size="small"
                    color="error"
                    :disabled="resolveRow(item).reserved"
                    @click="emit('uninstall', { extension: resolveRow(item) })"
                  >
          ...
          >
            <ItemCard
              ...
              @toggle-enabled="
                () =>
                  extension.activated
                    ? emit('plugin-off', extension)
                    : emit('plugin-on', extension)
              "
            >
              ...
              <template #actions>
                <v-menu location="top end">
                  ...
                  <v-list density="compact" min-width="180">
                    ...
                    <v-list-item
                      v-if="extension.repo"
                      @click="emit('view-readme', extension)"
                    >
                      <template #prepend>
                        <v-icon size="small">
                ...
                <v-btn
                  ...
                  color="primary"
                  variant="text"
                  @click.stop="emit('reload', extension.name)"
                >
                  <v-icon>mdi-refresh</v-icon>
                  <v-tooltip activator="parent" location="top">
                    {{ tm('tooltips.reload') }}
                ...
                <v-btn
                  ...
                  color="primary"
                  variant="text"
                  @click.stop="emit('open-config', extension.name)"
                >
                  <v-icon>mdi-cog</v-icon>
                  <v-tooltip activator="parent" location="top">
                    {{ tm('tooltips.configure') }}
    ...
    <UninstallConfirmDialog
      v-model="showUninstallDialogModel"
    ...
    <v-dialog v-model="configDialogModel" width="1000" scrollable>
      <v-card class="dialog-sticky-card">
        <v-card-title class="text-h5 dialog-sticky-title">
        ...
        <v-card-text class="dialog-sticky-content">
          <AstrBotConfig
            ...
            :metadata-key="currNamespace"
            :plugin-name="currNamespace"
          />
          <p v-else>
    ...
    <v-dialog v-model="forceUpdateDialogShowModel" max-width="500">
      <v-card class="rounded-lg">
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="emit('cancel-force-update')">
            {{ tm('buttons.cancel') }}
          </v-btn>
          <v-btn
            color="warning"
            variant="tonal"
            @click="emit('confirm-force-update')"
          >
      ...
      <v-card class="dialog-sticky-card">
        ...
        <v-card-text class="dialog-sticky-content">
          ...
          >
            ...
            <template #item.event_type="{ item }: { item: PluginHandlerInfo }">
              {{ item.event_type }}
            </template>
            <template #item.desc="{ item }: { item: PluginHandlerInfo }">
              {{ item.desc }}
            </template>
            <template #item.type="{ item }: { item: PluginHandlerInfo }">
              <v-chip color="success">
                {{ item.type }}
              </v-chip>
            </template>
            <template #item.cmd="{ item }: { item: PluginHandlerInfo }">
        ...
        <v-card-actions class="dialog-sticky-actions">
          ...
          <v-btn
            color="blue-darken-1"
            variant="text"
            @click="showPluginInfoDialogModel = false"
          >
            {{ tm('buttons.close') }}
          </v-btn>
        </v-card-actions>
...
const props = defineProps<{
  ...
  pluginHandlerInfoHeaders: Array<{
    title: string;
    key: string;
    [k: string]: unknown;
  }>;
...
const emit = defineEmits<{
  ...
  (
    e: 'uninstall',
    payload: { extension: InstalledPlugin; options?: UninstallOptions },
  ): void;
...
const getExtensionDisplayTitle = (row: unknown) => {
  const resolved = resolveRow(row);
  return (
    resolved.display_name?.length ? resolved.display_name : resolved.name
  ) as string;
};
...
const toItemCardExtension = (extension: InstalledPlugin) => {
  return {
    ...extension,
    _title: (extension.display_name && extension.display_name.length
      ? extension.display_name
      : extension.name) as string,
  };
};

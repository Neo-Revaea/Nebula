    ...
    ltmGetGraph(userId: string | null = null) {
      ...
      axios
        ...
        .then((response) => {
          ...
          const nodesRaw: NodeRaw[] =
            data && Array.isArray(data.nodes) ? (data.nodes as NodeRaw[]) : [];
          const edgesRaw: EdgeRaw[] =
            data && Array.isArray(data.edges) ? (data.edges as EdgeRaw[]) : [];
    ...
    ltmGetUserIds() {
      axios
        ...
        .then((response) => {
          // 确保返回的数据是数组类型
          const data = response.data.data;
          this.userIdList = Array.isArray(data)
            ? (data as unknown[]).map((x) => String(x))
            : [];
        })
        .catch((error) => {
    ...
    identifyParallelLinks(links: GraphLink[]) {
      ...
      linkMap.forEach((parallels) => {
        if (parallels.length > 1) {
          // 有多条平行边，分配不同曲率
          parallels.forEach(
            (item: { link: GraphLink; isForward: boolean }, index: number) => {
              // 根据边的数量计算适当的曲率
